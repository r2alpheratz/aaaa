 ...
 * @author EQ
 * @date 2014. 5. 1. 오전 10:11:01
 * Class description : 기본열차사용자조정최적화할당SVC
 */
@Service("com.korail.yz.ys.ba.YSBA003SVC")
public class YSBA003SVC {
	 ...
	 * Method description : 최적화할당 사용자조정 이력이 있는 기본열차를 조회한다.  
	 * @param param
	 * @return
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> selectListUsrCtlOtmzAlcBsTrn(Map<String, ?> param) {		
		...
		{
			resultList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListUsrCtlOtmzAlcBsTrn", inputDataSet);
		}
		else if("Y".equals(inputDataSet.get("TRN_NO_STDR")))
		{
			resultList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListUsrCtlOtmzAlcBsTrn2", inputDataSet);
		}
		//메시지 처리
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> selectListUsrCtlOtmzAlcBsTrnDtl(Map<String, ?> param) {		
		...
		ArrayList<Map<String, Object>> resultList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListUsrCtlOtmzAlcBsTrnDtl", inputDataSet);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> selectListOtmzAlcBsAplTrm(Map<String, ?> param) {		
		...
		ArrayList<Map<String, Object>> resultList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListOtmzAlcBsAplTrm", inputDataSet);
	...
	/**
	 * 사용자조정할당 복사등록 복사대상열차조회
	 * @author 김응규
	 * @date 2014. 5. 12. 오후 4:20:15
	 * Method description : 사용자조정할당 복사등록할 복사대상열차를 조회한다.
	 * @param param
	 * @return
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> selectListUsrCtlAlcCpyTgtTrn(Map<String, ?> param) {		
		...
		ArrayList<Map<String, Object>> resultList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListUsrCtlAlcCpyTgtTrn", inputDataSet);
	 ...
	 * Method description : 사용자조정할당수 수정
	 * @param param
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public Map<String, ?> updateUsrCtlAlcNum(Map<String, ?> param) {		
	    ...
	    for( int i = 0; i < usrCtlList.size() ; i++ )
	    {
	    	...
	    	{
	    		updateCnt += dao.update("com.korail.yz.ys.ba.YSBA003QMDAO.updateUsrCtlAlcNum", item);	
	    	}
	    	else if(item.get("DMN_PRS_DV_CD").equals("D"))
	    	{
	    		deleteCnt += dao.delete("com.korail.yz.ys.ba.YSBA003QMDAO.deleteUsrCtlAlcNum", item);
	    	}
	    }
	...
	/**
	 * @author 김응규
	 * @date 2014. 4. 21. 오전 11:34:18
	 * Method description : 사용자조정할당수 등록
	 * @param param
	 * @return
	 * @throws Exception e
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> insertUsrCtlAlcNum(Map<String, ?> param) throws Exception {		
		...
		if(!"NEW".equals(hstCreDv))  //신규인경우엔 조회해온적용일자를 숫자로 변환할 필요없음 ::: (조회하지 않고 입력할수도 있기 때문에 적용일자가 ""으로 넘어옴. 이를 숫자로 변환하면 NumberFormatException 발생함) 
		{
		...
		if("ADD".equals(hstCreDv))  //분할생성(추가등록)
		{
				...
				ArrayList<Map<String, Object>> hstCntList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectHstCntAdd", inputDataSet);
				...
				{
					ArrayList<Map<String, Object>> inputValidChkList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectListInputValidChk", inputDataSet);
						...
						for (int i = 0; i < inputList.size(); i++)
						{
							for (int j = 0; j < inputValidChkList.size(); j++)
							{
								String sInputData = inputList.get(i).get("DAY_DV_CD") +  inputList.get(i).get("PSRM_CL_CD") 
										+ inputList.get(i).get("ZONE_SEG_GP_NO") + inputList.get(i).get("BKCL_CD");
								String sCompData = (String) inputValidChkList.get(j).get("DAY_DV_CD")+inputValidChkList.get(j).get("PSRM_CL_CD")
										+ inputValidChkList.get(j).get("ZONE_SEG_GP_NO") + inputValidChkList.get(j).get("BKCL_CD");
		...
		else if("NEW".equals(hstCreDv)) //신규생성
		{
			...
			//1. 정합성 및 중복체크
			ArrayList<Map<String, Object>> hstCntList = (ArrayList) dao.list("com.korail.yz.ys.ba.YSBA003QMDAO.selectHstCnt", inputDataSet);
		...
		if ("ADD".equals(hstCreDv))  //분할생성(추가등록)
		{
			...
			if (aplStDt == inputAplStDt )
			{
				...
				if (aplClsDt == inputAplClsDt)
				{
					...
					for (int i = 0; i < inputList.size(); i++)
					{
						...
						LOGGER.debug("분할생성 사용자조정최적화할당수 등록 빡!!!!!!!!!!!!!!");
						insertCnt += dao.insert("com.korail.yz.ys.ba.YSBA003QMDAO.insertUsrCtlAlcNum", item);
					}
					if(insertCnt < 1)
					{
						throw new CosmosRuntimeException("WZZ000012", null);  //등록 작업이 실패하였습니다 - 입력값을 확인하십시오.
				...
				else if (aplClsDt > inputAplClsDt) 
				{
					...
					updateCnt = dao.update("com.korail.yz.ys.ba.YSBA003QMDAO.updateAplClsDt", inputDataSet);
					...
					insertCnt = dao.insert("com.korail.yz.ys.ba.YSBA003QMDAO.insertUsrCtlAlcAddCopy", inputDataSet);
			...
			else if (aplStDt < inputAplStDt)  //조회해온 적용시작일자가 입력받은적용시작일자보다 작은경우
			{
				...
				if (aplClsDt == inputAplClsDt) //조회해온 적용종료일자와 입력받은 적용종료일자가 같은경우 
				{
					...
					updateCnt = dao.update("com.korail.yz.ys.ba.YSBA003QMDAO.updateAplClsDt2", inputDataSet);
					...
					insertCnt += dao.insert("com.korail.yz.ys.ba.YSBA003QMDAO.insertUsrCtlAlcAddCopy2", inputDataSet);
					if(insertCnt < 1)
					{
						throw new CosmosRuntimeException("WZZ000012", null);  //등록 작업이 실패하였습니다 - 입력값을 확인하십시오.
					}
				...
				else if (aplClsDt > inputAplClsDt)
				{
					...
					insertCnt = dao.insert("com.korail.yz.ys.ba.YSBA003QMDAO.insertUsrCtlAlcAddCopy3", inputDataSet);
	 ...
	 * @author 김응규
	 * @date 2014. 4. 21. 오전 11:34:18
	 * Method description : 사용자조정할당수 복사등록
	 * @param param
	 * @return
	 * @throws Exception e
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Map<String, ?> insertUsrCtlAlcNumCopy(Map<String, ?> param) throws Exception {		
