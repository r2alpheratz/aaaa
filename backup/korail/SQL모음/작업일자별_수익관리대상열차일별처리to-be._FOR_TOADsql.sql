  <작업일자별조회>
   SELECT D.YMGT_JOB_ID AS YMGT_JOB_ID   /* 수익관리작업ID                         */
                                     ,
         ...
         DECODE (
            (SELECT 'Y'
              ...
              WHERE RUN_DT = A.RUN_DT AND TRN_NO = A.TRN_NO AND ROWNUM = 1),
            NULL, A.YMS_APL_FLG,
            'Y')
    ...
    FROM (SELECT A.RUN_DT AS RUN_DT                     /* 운행일자             */
            ...
            FROM TB_YYDK301 A                                    /* 열차기본TBL */
                 ...
                 (SELECT RUN_DT                     /* 운행일자                 */
                   ...
                   WHERE (   (    DECODE (PSRM_CL_CD || BKCL_CD,
                                          '1F1', 1,
                                          '1C1', 2,
                                          '1R1', 3,
                                          '1R2', 4,
         ...
         (SELECT A.YMGT_JOB_ID                      /* 수익관리작업ID             */
           ...
           WHERE     (D.JOB_DTTM, A.RUN_DT, A.TRN_NO) IN
                        (  SELECT MAX (B.JOB_DTTM), A.RUN_DT, A.TRN_NO
                             FROM (SELECT A.YMGT_JOB_ID         /* 수익관리작업ID */
                                                       , A.RUN_DT /* 운행일자         */
                                                                 , A.TRN_NO /* 열차번호         */
                                     FROM TB_YYFD011 A     /* 수익관리대상열차내역TBL */
                                    WHERE     A.RUN_DT BETWEEN :RUN_TRM_ST_DT
                                                           AND :RUN_TRM_CLS_DT
                                  ...
                                  (SELECT B.YMGT_JOB_ID         /* 수익관리작업ID */
                                    ...
                                    WHERE     B.REG_DTTM LIKE :JOB_DT || '%'
                                          AND (   :ONLN_ARNG_DV_CD IS NULL
                                               OR B.ONLN_ARNG_DV_CD =
                                                     :ONLN_ARNG_DV_CD) /* 온라인배치구분코드 */
                                                                      ) B
                            WHERE B.YMGT_JOB_ID = A.YMGT_JOB_ID
                         GROUP BY A.RUN_DT, A.TRN_NO)
                 AND A.YMGT_JOB_ID = D.YMGT_JOB_ID
                 AND A.REG_DTTM LIKE :JOB_DT || '%') D
   WHERE     A.RUN_DT = D.RUN_DT
         ...
         AND (A.STLB_TRN_CLSF_CD, A.ROUT_CD, A.UP_DN_DV_CD, A.DPT_TM) IN
                (SELECT TRN_CLSF_CD                         /* 열차종별코드       */
                  ...
                  WHERE     USR_GP_ID IN
                               (SELECT DISTINCT USR_GP_ID        /* 사용자그룹ID */
