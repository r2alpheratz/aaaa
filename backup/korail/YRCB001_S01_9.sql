  SELECT /*+ com.korail.yz.yr.cb.YRCB001QMDAO.selectListRunDtPrAcvmCompQry */
        SUBSTR (A.RUN_DT, 1, 4)
         || '.'
         || SUBSTR (A.RUN_DT, 5, 2)
         || '.'
    ...
    FROM (  SELECT A.RUN_DT
              ...
              FROM (SELECT B.RUN_DT
                  ...
                  ,(  SELECT A.RUN_DT
                            ,A.TRN_NO
                            ,A.PSRM_CL_CD
                            ,DECODE (:BKCL_CD, 'F2', 'F2', A.BKCL_CD) AS BKCL_CD
             ...
             WHERE A.RUN_DT = B.RUN_DT
               ...
               AND A.RUN_DT = C.RUN_DT
               AND C.DAY_DV_CD = DECODE (:DAY_DV_CD, '0', C.DAY_DV_CD, :DAY_DV_CD)
          GROUP BY A.RUN_DT, A.TRN_NO, D.BS_SEAT_NUM) A
        ,TB_YYDK002 B
   WHERE A.RUN_DT = B.RUN_DT
GROUP BY A.RUN_DT, B.DAY_DV_CD
ORDER BY A.RUN_DT
